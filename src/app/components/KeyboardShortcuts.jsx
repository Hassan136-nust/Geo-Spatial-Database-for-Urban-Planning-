  ...
  return (
    <>
      <AnimatePresence>
        {!isVisible && (
          <motion.button
            ...
            className="fixed bottom-8 left-8 z-50 w-12 h-12 backdrop-blur-xl bg-white/5 border border-white/10 rounded-full flex items-center justify-center hover:bg-white/10 transition-colors group"
            title="Keyboard Shortcuts"
          >
            <Keyboard className="w-5 h-5 text-white/60 group-hover:text-white/90 transition-colors" />
          </motion.button>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {isVisible && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsVisible(false)} className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm" />
                      ...
                      <motion.div key={index} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.05 }} className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                          {shortcut.keys.map((key, i) => (
                            <kbd key={i} className="px-3 py-1.5 text-sm font-mono bg-white/10 border border-white/20 rounded-lg">{key}</kbd>
                          ))}
                        </div>
                      </motion.div>
                    ))}
